...
func StartServer(cfg config.Config) {
	...
	for i, key := range cfg.Keys {
		// Another way to instantiate the circuit: prover.SetupInsertion(Depth, NumberOfUtxos)
...
func TestInsertionHappyPath26_1(t *testing.T) {
	...
	response, err := http.Post("http://localhost:8080/prove", "application/json", strings.NewReader(jsonString))
	if err != nil {
		t.Fatal(err)
	}
...
func TestInsertionHappyPath26_2(t *testing.T) {
...
func TestInsertionHappyPath26_1_JSON(t *testing.T) {
	testInput := `
...
`
	...
	response, err := http.Post("http://localhost:8080/prove", "application/json", strings.NewReader(testInput))
	if err != nil {
		t.Fatal(err)
	}
	if response.StatusCode != http.StatusOK {
...
func TestInsertionWrongPath26_1_JSON(t *testing.T) {
	testInput := `
	{"root":["0x0"],
	"inPathIndices":[0],
	"inPathElements":[
	...
	logging.Logger().Debug().Msg("[TestInsertionWrongPath22_1_JSON] end")
}
