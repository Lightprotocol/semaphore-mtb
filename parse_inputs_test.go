...
func TestParseInputs(t *testing.T) {
	t.Skip("skipping TestParseInputs")
	fmt.Println("[TestParseInputs] begin")
	...
	proofData, err := prover.ParseInput(inputHex)
	if err != nil {
		t.Errorf("error parsing input: %v", err)
	}
	fmt.Println("Root[0]: ", proofData.Root[0])
	fmt.Println("Leaf[0]: ", proofData.Leaf[0])

	response, err := http.Post("http://localhost:8080/prove", "application/json", strings.NewReader(inputHex))
	if err != nil {
		t.Fatal(err)
	}
	...
	fmt.Println("[TestParseInputs] end")
}

func TestEmptyTree(t *testing.T) {
	fmt.Println("[TestEmptyTree] begin")
	tree := tree.NewTree(22)
	...
	inPathElementsStr := make([]string, len(inPathElements))
	for i, bi := range inPathElements {
		inPathElementsStr[i] = bi.String()
	}

	fmt.Println("Root: ", root.String())
	fmt.Println("Leaf: ", leaf.String())
	fmt.Println("inPathIndices: ", inPathIndices)
	fmt.Println("inPathElements: ", inPathElementsStr)
	fmt.Println("[TestEmptyTree] end")
}
